...
namespace Vault2Door
{
    public partial class Form1
    {
        ...
        private void ShowChart(string key)
        {
            ...
            if (key.Contains("gold"))
            {
                ...
                var line = new LineSeries<double>
                {
                    ...
                    Fill = new LinearGradientPaint(new[] { new SKColor(255, 215, 0, 80), new SKColor(255, 215, 0, 10) }, new SKPoint(0, 0), new SKPoint(0, 1))
                };
            ...
            else if (key.Contains("bronze"))
            {
                ...
                {
                    Values = values, MaxBarWidth = double.NaN,
        ...
        private void ApplyChartAxesTheme()
        {
            ...
            chart.XAxes = new[] { new LiveChartsCore.SkiaSharpView.Axis { LabelsPaint = new LiveChartsCore.SkiaSharpView.Painting.SolidColorPaint(labels), SeparatorsPaint = new LiveChartsCore.SkiaSharpView.Painting.SolidColorPaint(grid) { StrokeThickness = 1 }, TicksPaint = new LiveChartsCore.SkiaSharpView.Painting.SolidColorPaint(grid) { StrokeThickness = 1 }, TextSize = 12 } };
            chart.YAxes = new[] { new LiveChartsCore.SkiaSharpView.Axis { LabelsPaint = new LiveChartsCore.SkiaSharpView.Painting.SolidColorPaint(labels), SeparatorsPaint = new LiveChartsCore.SkiaSharpView.Painting.SolidColorPaint(grid) { StrokeThickness = 1 }, TicksPaint = new LiveChartsCore.SkiaSharpView.Painting.SolidColorPaint(grid) { StrokeThickness = 1 }, TextSize = 12 } };

            // ensure toggle visibility in dark mode
            if (chkRealtime != null)
            {
                chkRealtime.ForeColor = isDarkMode ? Color.Gainsboro : Color.Black;
                chkRealtime.BackColor = isDarkMode ? Color.FromArgb(24, 27, 32) : Color.White;
            }
        }
    }
}
